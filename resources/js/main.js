function enhanceCodeSnippets() {
    ...
    $('pre > code').each(function () {
        ...
        if ($pre.parent().hasClass('noton-code-snippet')) {
            return;
        }

        const $wrapper = $('<div class="noton-code-snippet relative"></div>');
        $pre.wrap($wrapper);
        $pre.addClass('pe-12');

        const $button = $(
            '<button class="absolute top-[5px] end-[5px] p-1.5 !m-0 text-gray-600 hover:text-gray-400 transition">' +
...
$(document).ready(function () {
    ...
    Livewire.hook('commit', ({ succeed }) => {
        succeed(() => setTimeout(enhanceCodeSnippets, 0));
    });
});
